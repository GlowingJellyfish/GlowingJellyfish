...
template<>
struct std::hash<GlowingJellyfish::Eval::NNUE::EvalFile> {
    std::size_t operator()(const GlowingJellyfish::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        GlowingJellyfish::hash_combine(h, evalFile.defaultName);
        GlowingJellyfish::hash_combine(h, evalFile.current);
        GlowingJellyfish::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
