...
struct std::hash<GlowingJellyfish::FixedString<N>> {
    std::size_t operator()(const GlowingJellyfish::FixedString<N>& fstr) const noexcept {
        return GlowingJellyfish::hash_bytes(fstr.data(), fstr.size());
    }
};
