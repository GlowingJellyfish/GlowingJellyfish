...
class TSAN:
    ...
    def set_tsan_option():
        with open(f"tsan.supp", "w") as f:
            f.write(
                """
race:GlowingJellyfish::TTEntry::read
race:GlowingJellyfish::TTEntry::save
race:GlowingJellyfish::TranspositionTable::probe
race:GlowingJellyfish::TranspositionTable::hashfull
"""
            )
...
class GlowingJellyfish:
    ...
    def _check_process_alive(self):
        if not self.process or self.process.poll() is not None:
            print("\n".join(self.output))
            raise RuntimeError("GlowingJellyfish process has terminated")
    ...
    def send_command(self, command: str):
        if not self.process:
            raise RuntimeError("GlowingJellyfish process is not started")
